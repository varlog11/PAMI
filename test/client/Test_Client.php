...
*/
        ...
        public function can_async_agi()
        {
            ...
            setFgetsMock($standardAMIStart, $write);
            $client = new \PAMI\Client\Impl\ClientImpl($options);
            $listener = new MyListenerTest($options);
            $listener->run();
            $event = array(
            'Event: AsyncAGIStartEvent',
            ''
            );
            setFgetsMock($event, $event);
            //$event = MyListenerTest::$event;
            $client->process();
            $this->assertTrue($client instanceof \PAMI\Client\Impl\ClientImpl);
            $this->assertTrue($listener instanceof MyListenerTest);
        }        
    }
    ...
    class MyListenerTest implements IEventListener
    {
        private $_client;
        private $_id;
